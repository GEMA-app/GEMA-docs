...
│       │   ├── entities/             # Entidades con comportamiento y reglas (User, Company, Role...)
│       │   ├── value_objects/        # Objetos de valor inmutables (Email, Slug, Identifiers...)
│       │   ├── exceptions/           # Excepciones personalizadas por módulos
...
│       │   ├── use_cases/            # Casos de uso estructurados por módulo (auth, company, role...)
│       │   └── dtos/                 # DTOs de entrada y salida (auth_dtos, company_dtos...)
│       │
...
│       │   │   └── models/           # Modelos ORM (user, company, role, location, asset...)
│       │   ├── repositories/         # Implementaciones de repositorios (user, company, role...)
│       │   ├── cache/
...
│       │   ├── api/
│       │   │   └── v1/
│       │   │       ├── endpoints/    # Routers (auth, companies, roles, locations, assets)
│       │   │       ├── schemas/      # Schemas JSON:API (auth, company, role, location, asset)
│       │   │       └── router.py     # v1_router con prefijo /v1
│       │   ├── exception_handlers/   # Mapeo excepción → HTTP + JSON:API por archivos
│       │   └── middlewares/
...
- **`entities/`**: Paquete que contiene las entidades del dominio con su propio comportamiento y lógica (User, Company, Role, Location, Asset...).
- **`value_objects/`**: Paquete que alberga objetos inmutables que representan conceptos del negocio mediante valores (Email, Slug, identifiers...).
- **`exceptions/`**: Paquete que define la jerarquía de excepciones propias del negocio.
...
- **`ports/`**: Aquí se definen los puertos (interfaces estructuradas mediante `Protocol` en Python) que la infraestructura debe implementar (ej. `UserRepositoryPort`, `UnitOfWorkPort`, etc.).
- **`use_cases/`**: Contiene los casos de uso organizados en carpetas por módulos del negocio.
- **`dtos/`**: Define los DTOs de entrada y salida (`dataclasses` puros) para la capa de aplicación, organizados por archivos.
...
- **`db/`**: Define la sesión asíncrona de base de datos y los modelos ORM de SQLAlchemy estructurados como paquete de modelos.
...
- **`exception_handlers/`**: Mapea las excepciones de la aplicación y del dominio a respuestas HTTP estandarizadas en archivos segregados.
