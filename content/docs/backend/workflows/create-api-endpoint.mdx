...
El `__init__.py` re-exporta los dos Use Cases para que el Composition Root pueda importarlos desde `app.application.use_cases.preferences` sin acoplarse a la estructura interna del paquete.
...
Registrar el router de preferencias en el `v1_router` con el prefijo `/companies/{company_id}/me/preferences`. La ruta no tiene `{id}` porque preferences es un singleton por usuario.
...
Las fábricas del Composition Root cablean las dependencias. `provide_user_preferences_use_case` (lectura) y `get_update_preferences_use_case` (escritura) reciben el UoW via `Depends(get_uow)` y construyen el Use Case correspondiente. No hay lógica de negocio, solo instanciación.
...
- Valida que `company_id` del path coincida con la empresa del token (tenant check).
...
- **Sin control de concurrencia**: PATCH no tiene optimistic locking (lost updates posibles en alta concurrencia).
- **Composition Root acoplado a FastAPI**: Impide testear use cases sin FastAPI.
- **Testing y Observabilidad**: descartados por directiva.
...
- [ ] 5. Modelo ORM: hereda de TenantMixin y TimestampMixin
...
- [ ] 11. RBAC: `require_permission()` presente y módulo registrado en `PermissionModule` (valor español)
