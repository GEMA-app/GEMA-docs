...
<Callout type="info" title="¿Compose V2 vs Compose V1?">
El Compose original era un binario independiente (`docker-compose`). Desde Docker 23, Compose es un plugin oficial del cliente de Docker y se invoca con `docker compose` (sin guion). La diferencia no es estética: V2 soporta perfiles, verificaciones de salud con condiciones (`service_healthy`) y sintaxis YAML que V1 ignora. Si la terminal devuelve `v2.x`, el entorno es compatible.
</Callout>
...
<Callout type="info" title="Lista de verificación rápida">
