...
```python
from src.app.infrastructure.config.settings import get_settings
...
| `APP_TITLE` | Título del proyecto (aparece en `/docs`) | `GEMA Backend` |
...
En cualquier parte de la aplicación puedes obtener la instancia de configuración mediante la función `get_settings()`:
